  ...
  return (
    <ScrollShadow hideScrollBar className="min-h-0 flex-1 overflow-y-auto">
      <div className="flex flex-col gap-5 py-1">
        {/* Role */}
        <ProfileSection icon="UserCircle" label={t('Role')}>
          {isCustom ? (
            <EditableField
              value={agent.role}
              placeholder={t('Describe the agent\u2019s role...')}
        ...
        {/* Instructions */}
        <ProfileSection icon="PageEdit" label={t('Instructions')}>
          {isCustom ? (
            <EditableTextarea
              value={agent.instructions || ''}
              placeholder={t('Write the agent\u2019s system instructions...')}
        ...
        {examples && examples.length > 0 && (
          <ProfileSection icon="ChatBubble" label={t('Example prompts')}>
  ...
  return (
    <div className="-mx-1 flex flex-col gap-1.5">
      ...
      <p className="text-muted text-[10px]">
        {t('Esc to cancel \u00b7 click outside to save')}
      </p>
    </div>
  )
}
