...
export const ThreadList = memo(function ThreadList({
  ...
  const layoutOptions: { id: CollectionLayout; label: string; icon: 'TableRows' | 'KanbanBoard' }[] = [
    { id: 'list', label: t('List'), icon: 'TableRows' },
    { id: 'board', label: t('Board'), icon: 'KanbanBoard' },
  ]
  ...
  const layoutToggle = onLayoutChange ? (
    <Dropdown>
      <Button isIconOnly size="sm" variant="ghost" aria-label={t('Change layout')}>
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col text-left">
        <span className="text-foreground text-sm font-medium">
          {t(createNewLabel as any)}
        </span>
        <span className="text-muted text-xs">{t(createNewSublabel as any)}</span>
      </div>
    </button>
  ) : undefined
  ...
  return (
    <CollectionView
      ...
      search={search}
      onSearchChange={onSearchChange}
      searchPlaceholder={t('Search tasks and conversations…')}
