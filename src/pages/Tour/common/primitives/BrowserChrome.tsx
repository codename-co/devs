/**
 * BrowserChrome — a fake browser window frame for use in video scenes.
 *
 * Renders a macOS-style browser chrome with traffic lights, a tab, and a
 * URL bar. Two positioning strategies:
 *
 * 1. **Explicit coords** — pass `x`, `y`, `width`, `height` for absolute
 *    positioning in the parent's coordinate system.
 * 2. **Responsive fill** — omit those and (optionally) pass `inset` (px).
 *    The chrome fills its parent, auto-centred up to a 1280×720 cap.
 */
...
interface BrowserChromeProps {
  ...
  x?: number
  y?: number
  /** Uniform inset (px) from the parent when filling. Default: 0. */
  inset?: number
  url?: string
  dark?: boolean
  children?: ReactNode
  ...
  tabTitle?: string
  /** Accent color for the tab icon dot. Default: `oklch(62.04% 0.195 253.83)`. */
  accentColor?: string
}
...
export function BrowserChrome({
  ...
  ty = 0,
  tabTitle = 'DEVS',
  accentColor = 'oklch(62.04% 0.195 253.83)',
}: BrowserChromeProps) {
