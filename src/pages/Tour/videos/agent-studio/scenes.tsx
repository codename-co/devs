...
function SceneBrowserAgentsInner() {
  ...
  // Cursor target: the real "New agent" button inside the AgentCollection.
  // Resolved each frame so the cursor tracks the real DOM position even when
  // the panel reflows for narrow viewports or different zoom levels. Falls
  // back to a hand-tuned fraction before the panel has mounted.
  const newBtnFrac = { x: 0.22, y: 0.22 }
  const cursorStartPos = { x: stageW * 0.5, y: stageH * 0.7 }
  const cursorEndPos =
    resolveAnchor?.('[data-tour-anchor="new-agent"]') ??
    { x: newBtnFrac.x * stageW, y: newBtnFrac.y * stageH }
  ...
  const previewT = Easing.easeOutCubic(
    clamp((time - PREVIEW_START) / 0.6, 0, 1),
  )
