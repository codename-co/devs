...
describe('User Identity', () => {
  ...
  describe('signData / verifySignature', () => {
    it('should sign and verify data correctly', async () => {
      ...
      const isValid = await verifySignature(identity.publicCryptoKey, signature, data)
    ...
    it('should fail verification with wrong data', async () => {
      const identity = await generateUserIdentity()
      const data = new TextEncoder().encode('test message').buffer as ArrayBuffer
      const wrongData = new TextEncoder().encode('wrong message').buffer as ArrayBuffer
      
      const signature = await signData(identity.privateCryptoKey, data)
      const isValid = await verifySignature(identity.publicCryptoKey, signature, wrongData)
    ...
    it('should fail verification with wrong key', async () => {
      ...
      const isValid = await verifySignature(identity2.publicCryptoKey, signature, data)
...
describe('Device Identity', () => {
  describe('generateDeviceIdentity', () => {
    it('should generate device identity linked to user', async () => {
      ...
      const { device, privateKey } = await generateDeviceIdentity(userId, 'Test Device')
