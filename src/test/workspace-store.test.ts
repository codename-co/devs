...
describe('Workspace Store', () => {
  ...
  describe('invite links', () => {
    ...
    it('should use invite link and increment count', () => {
      ...
      const updatedLink = useWorkspaceStore.getState().workspaces[0].inviteLinks[0]
    ...
    it('should reject link when max uses reached', () => {
      const store = useWorkspaceStore.getState()
      const workspace = store.createWorkspace('Test', 'owner')
      const link = store.createInviteLink(workspace.id, 'viewer', { maxUses: 1 })
  ...
  describe('sharing', () => {
    it('should share and unshare agent', () => {
      ...
      expect(useWorkspaceStore.getState().workspaces[0].sharedAgents).toContain('agent-123')
      
      store.unshareAgent(workspace.id, 'agent-123')
      expect(useWorkspaceStore.getState().workspaces[0].sharedAgents).not.toContain('agent-123')
    })
    ...
    it('should not duplicate shared agents', () => {
      ...
      expect(useWorkspaceStore.getState().workspaces[0].sharedAgents).toHaveLength(1)
    })
  })
