...
describe('Existing Task Orchestration', () => {
  ...
  it('should use existing task instead of creating new one when existingTaskId is provided', async () => {
    ...
    mockGetTaskById.mockResolvedValue(existingTask).mockResolvedValueOnce(existingTask).mockResolvedValueOnce(updatedTask)

    // Act
    const result = await WorkflowOrchestrator.orchestrateTask(prompt, existingTaskId)
    ...
    expect(mockUpdateTask).toHaveBeenCalledWith(existingTaskId, expect.objectContaining({
      complexity: 'simple',
      estimatedPasses: 1
    }))
    ...
    // Should have retrieved the existing task twice (before update and after update)
  ...
  it('should create new task when existingTaskId is not provided (fallback)', async () => {
    ...
    expect(mockUpdateTask.mock.calls[0][1]).toEqual(expect.objectContaining({
      status: 'in_progress',
      assignedAgentId: 'agent-1'
    }))
    expect(mockUpdateTask.mock.calls[1][1]).toEqual(expect.objectContaining({
      status: 'completed'
    }))
  })
  ...
  it('should throw error if existingTaskId is provided but task not found', async () => {
    ...
    // Act & Assert
    await expect(WorkflowOrchestrator.orchestrateTask(prompt, nonExistentTaskId))
