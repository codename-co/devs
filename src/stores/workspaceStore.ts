...
interface WorkspaceActions {
  ...
  // Invite links
  createInviteLink: (workspaceId: string, role: 'editor' | 'viewer', options?: {
    expiresAt?: Date
    maxUses?: number
  }) => InviteLink
  ...
  persist(
    (set, get) => ({
        ...
        set((state) => ({
          workspaces: state.workspaces.map((w) =>
            w.id === workspaceId
              ? {
                  ...w,
                  members: [...w.members.filter(m => m.userId !== member.userId), member],
                  updatedAt: new Date()
                }
              : w
          )
      ...
      shareConversation: (workspaceId: string, conversationId: string) => {
        set((state) => ({
          workspaces: state.workspaces.map((w) =>
            w.id === workspaceId && !w.sharedConversations.includes(conversationId)
              ? {
                  ...w,
                  sharedConversations: [...w.sharedConversations, conversationId],
                  updatedAt: new Date()
                }
              : w
          )
        ...
        set((state) => ({
          workspaces: state.workspaces.map((w) =>
            w.id === workspaceId
              ? {
                  ...w,
                  sharedConversations: w.sharedConversations.filter((id) => id !== conversationId),
                  updatedAt: new Date()
                }
              : w
          )
        ...
        set((state) => ({
          workspaces: state.workspaces.map((w) =>
            w.id === workspaceId
              ? {
                  ...w,
                  sharedKnowledge: w.sharedKnowledge.filter((id) => id !== knowledgeId),
                  updatedAt: new Date()
                }
              : w
          )
        ...
        set((state) => ({
          workspaces: state.workspaces.map((w) =>
            w.id === workspaceId
              ? {
                  ...w,
                  syncProviders: w.syncProviders.filter((id) => id !== providerId),
                  updatedAt: new Date()
                }
              : w
          )
...
export const useActiveWorkspace = () => useWorkspaceStore((state) => state.getActiveWorkspace())
