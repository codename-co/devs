...
interface SyncStoreActions {
  ...
  getWorkspaceSyncStatus: (
    workspaceId: string,
  ) => WorkspaceSyncState | undefined
  updateWorkspaceStatus: (
    workspaceId: string,
    status: Partial<WorkspaceSyncState>,
  ) => void
...
export const useSyncStore = create<SyncStore>()((set, get) => ({
  ...
  registerProvider: (provider: SyncProvider) => {
    ...
    events.forEach((event) => {
      provider.on(event, (data) => {
        ...
        if (event === 'connected') {
          ...
          const anyConnected = Array.from(providers.values()).some((p) =>
            p.isConnected(),
          )
    ...
    get().logEvent('provider_registered', {
  ...
  syncWorkspace: async (workspaceId: string) => {
    ...
    get().updateWorkspaceStatus(workspaceId, {
      status: {
        ...state.workspaceSyncStates.get(workspaceId)?.status!,
        state: 'syncing',
      },
    })
...
export const useGlobalSyncStatus = () =>
  useSyncStore((state) => state.globalStatus)
...
export const useSyncProviders = () =>
  useSyncStore((state) => Array.from(state.providers.values()))
