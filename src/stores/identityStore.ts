    ...
    (set, get) => ({
      ...
      createIdentity: async (displayName?: string) => {
        ...
        try {
          ...
          const { device, privateKey: devicePrivateKey } =
          ...
          const exportedIdentity = await exportIdentity(
            identity,
            internalPassword,
          )
          ...
          const deviceKeyData = await crypto.subtle.exportKey(
            'pkcs8',
            devicePrivateKey,
          )
          const deviceKeyBase64 = btoa(
            String.fromCharCode(...new Uint8Array(deviceKeyData)),
          )
      ...
      addLinkedDevice: (device: DeviceIdentity) => {
        set((state) => ({
          linkedDevices: [
            ...state.linkedDevices.filter((d) => d.id !== device.id),
            device,
          ],
        }))
      },
...
export const useLinkedDevices = () =>
  useIdentityStore((state) => state.linkedDevices)
